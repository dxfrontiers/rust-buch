 ...
 */
    ...
    */
    ...
    // Bonus: move und borrow semantics
    // Notice that function_owned prints a different address than the other two

    fn function_borrow(variable: &u32){
        println!("function_borrow: \t {:p}", variable)
    }
    fn function_owned(variable: u32) -> u32{
        println!("function_owned: \t {:p}", &variable);
        variable*2
    }
    ...
    fn test_semantics(){
        let variable = 7u32;
        println!("test_semantics: \t {:p}", &variable);
        function_borrow(&variable);
        function_owned(variable);
    }


    fn print_string_pointer_and_data_location(s: &String){
        print!(" String: \t {:p} \t", s);
        println!("Slice:  \t {:p}", &s[..]);
        // println!("Slice: \t {:p}", s.into_boxed_str());
    }

    #[test]
    fn test_semantics_with_string(){
        let mut s1 = String::from("Test");
        let s2 = String::from("Lorem");
        print!("S1: ");
        print_string_pointer_and_data_location(&s1);
        print!("S2: ");
        print_string_pointer_and_data_location(&s2);

        std::mem::replace(&mut s1,s2);
